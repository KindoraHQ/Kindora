...
/// Mindora (audit-fixed test copy of Kindora)
/// Changes vs Kindora:
/// - CEI in swapBack (effects before interactions)
/// - No direct BNB send to charity inside swapBack (pull-payment)
/// - swapBack executed AFTER final transfer
/// - fees initialized
/// - addLiquidityETH return values used
/// - zero-check in setMultisig
/// - rescueETH hardened with nonReentrant + balance check
...
contract Mindora is ERC20, Ownable, ReentrancyLite {
    ...
    function rescueETH(uint256 amount) external nonReentrant {
        require(msg.sender == owner() || msg.sender == multisig, "not authorized");
        require(!rescuesLocked, "rescues locked");
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 ethAmount, address lpReceiver) private {
        ...
        (uint amountToken, uint amountETH, ) = uniswapV2Router.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
            0,
            0,
    ...
    function swapBack() private nonReentrant {
        ...
        uint256 charityTokens = contractBalance - liqTokens;

        // -------- CEI (Effects before Interactions) --------
        if (liqTokens >= tokensForLiquidity) tokensForLiquidity = 0;
        else tokensForLiquidity -= liqTokens;

        if (charityTokens >= tokensForCharity) tokensForCharity = 0;
        else tokensForCharity -= charityTokens;
        // --------------------------------------------------

        uint256 halfLiq = liqTokens / 2;
        uint256 toSwapForETH = charityTokens + halfLiq;

        uint256 initialETH = address(this).balance;
        if (toSwapForETH > 0) _swapTokensForEth(toSwapForETH);
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        // ✅ final transfer first
        uint256 sendAmount = amount - fees;
        super._transfer(from, to, sendAmount);

        // ✅ swapBack after balances updated
        uint256 contractBalance = balanceOf(address(this));
